...
class AttributeMovingAverageP(AttributeAbstract):
    ...
    def get_calculate_sigma(self, item: int) -> float:
        sigmas = [(self.get_average()*(1 - self.get_average())/self.data.table[i]['size'])**0.5 for i in range(len(self.data.table))]
    ...
    def values_for_plot(self) -> Sequence[float]:
        p_values = [item[self.config.selected_parameter]/item['size'] for item in self.data.table]
        result = []
